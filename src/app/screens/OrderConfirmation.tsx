        ...
        {!!s.items?.length && (
          <div className="space-y-2">
            {s.items.map(it => (
              <div key={it.id} className="flex items-center gap-3">
                ...
                onError={(e) => {
                    (e.currentTarget as HTMLImageElement).src = imagePath("placeholder.svg");
                  }}
                />
         ...
         <div className="grid gap-2 sm:grid-cols-2">
          <div className="rounded-lg border p-3">
            ...
            {!!(s.subtotal !== undefined) && (
              <>
                ...
                <div className="flex justify-between text-sm">
                  <span>Shipping</span>
                  <span>{s.shipping ? money(s.shipping) : "free"}</span>
                </div>
                <div className="flex justify-between text-sm">
                  <span>Tax</span>
                  <span>{money(s.tax ?? 0)}</span>
                </div>
              </>
            )}
          ...
          <div className="rounded-lg border p-3">
            ...
            <div>
              UPS Tracking: <strong>{s.tracking ?? "Pending"}</strong>
            </div>
            {s.shipTo && (
